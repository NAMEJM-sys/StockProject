...
def ema_calculation1(stockData ,EMA):
    ...
    current_ema20 = EMA.iloc[-1]['EMA_20']
    current_close = stockData.iloc[-1]['close']

    score = 5
    if current_ema20 > current_close:
        score = 9
    elif current_ema20 < current_close:
        score = 1
    else:
        score = 5
...
def ema_calculation2(EMA, period=5):
    ema_trend = EMA['EMA_20'].iloc[-period:].values
...
def ema20_calculation_all(stockData, EMA):
